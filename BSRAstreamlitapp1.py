# -*- coding: utf-8 -*-
"""
Created on Sun Oct 23 16:37:31 2022
...
model = load_model("C:/Users/navne/.spyder-py3/BSRAmodel1.h5")
...
Trim_LBP=st.number_input("Trim/LBP",step=1e-4, format="%.5f")
Trim_LBP_scaled=(Trim_LBP-0.0252/0.0146)
Length_Disp=st.number_input("Length/Disp",step=1e-4, format="%.2f")
Length_Disp_scaled=(Length_Disp-4.877)/0.216
L_B=st.number_input("L/B",step=1e-3, format="%.3f")
L_B_scaled=(L_B-5.399)/0.356
Block_Coefft=st.number_input("Block Coefft",step=1e-4, format="%.3f")
Block_Coefft_scaled=(Block_Coefft-0.562)/0.022
B_T=st.number_input("B/T",step=1e-4, format="%.3f")
B_T_scaled=(B_T-2.256)/0.259
LCB_Posn=st.number_input("LCB Posn",step=1e-4, format="%.3f")
LCB_Posn_scaled=(LCB_Posn+1.692)/1.836
V_L=st.number_input("V/Sqrt(L)",step=1e-4, format="%.3f")
